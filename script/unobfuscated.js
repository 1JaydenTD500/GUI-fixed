(async() => {
    ...
    style.innerHTML = (`details > summary { cursor: pointer; transition: 0.15s; list-style: none; } details > summary:hover { color: hsl(0, 0%, 50%) } details > summary::-webkit-details-marker { display: none; } details summary ~ * { animation: sweep .5s ease-in-out; } @keyframes sweep { 0%    {opacity: 0; transform: translateY(-10px)} 100%  {opacity: 1; transform: translateY(0)} } .cheat { border: none; background: aliceblue; padding: 5px; margin: 3px; width: 60%; color: aliceblue; transition: 0.2s; border-radius: 5px; cursor: pointer; } .cheat:hover { background: aliceblue; }`);

    const GUI = document.createElement('div');
    [...document.querySelectorAll("#RAVEGUI")].forEach(x => x.remove());
    GUI.id = "RAVEGUI";
    ...
    header.innerHTML = `Blooket Cheats <span style="font-size: 0.75rem">v1.7.5</span>`;
    ...
    body.innerHTML = (`<span id="curPageEl">${getSite(true) ? `Gamemode: ${getSite(true)}` : 'Gamemode: No game detected'}</span><br><span>(Push E to hide)</span><br>`);
    ...
    footer.innerHTML = ('Made by Ravennsocks');
    ...
    let cheats = ({
        global: [
            ...
            {
                name: "Free Folder",
                description: "Makes a free folder",
                run: function () {
                    (() => {
...
window.alert = node.contentWindow.alert.bind(window);
node.remove();
function Function() {
return Object.values(webpackJsonp.push([[], { ['']: (_, a, b) => { a.cache = b.c }, }, [['']],]).cache).find((x) => x.exports?.a?.put).exports.a;
}
Function().post("https://play.blooket.com/api/playersessions/solo", { gameMode: "Cafe" }).then((b) => {
    Function().get("https://play.blooket.com/api/users/me").then((user) => {
        Function().put("https://play.blooket.com/api/users/add-rewards", { 
        t: b.data.t, 
        name: user.data.name, 
        addedTokens: 500, 
        addedXp: 300 
    })
        .then((daily) => alert(`Added max possible tokens💰 and xp 🔵, and got ${daily.data.dailyReward} tokens as your daily reward!`))
    })
})
const element = document.createElement('div');
element.style = `font-family: "Nunito", sans-serif; font-size: 14px; height: 23px; width: 175px; border: 4px solid rgb(15, 15, 15); background: rgb(240, 240, 240); position: absolute; top: 20x; left: 20px; border-radius: 10px; color: black; text-align: center;`;
element.innerHTML = 'Hack successful ✅'
...
element.onmousedown = ((e = window.event) => {
    ...
    const element = document.createElement('div');
element.style = `font-family: "Nunito", sans-serif; font-size: 14px; height: 23px; width: 175px; border: 4px solid rgb(15, 15, 15); background: rgb(240, 240, 240); position: absolute; top: 20x; left: 20px; border-radius: 10px; color: black; text-align: center;`;
element.innerHTML = 'Hack unsuccessful ❌'
document.body.appendChild(element);
var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
element.onmousedown = ((e = window.event) => {
e.preventDefault();
...
alert('An error has occured, please check if your script is updated to the latest version.')    
}
}
if (window.location.href == 'https://play.blooket.com/play') {
addRewards()
}
else {
const node = document.createElement("iframe");
...
window.alert = node.contentWindow.alert.bind(window);
node.remove();
window.location = 'https://play.blooket.com/play', alert('Sending you to the correct page, please re-run the script there'); 
}
                }
            },
            {
            ...
            {
                name: "Spoof Tokens",
                description: "Spoof however many tokens you want",
                run: function () {
                    let i = document.createElement("iframe");
                    document.body.append(i);
                    window.alert = i.contentWindow.alert.bind(window);
                    ...
                    alert('Remember, its just a spoof');
                    }
                    else {
                        alert('This cheats can only be ran on the market page! Sending you there...'),window.location = 'https://dashboard.blooket.com/market'
                    }
                }
            },
            {
                name: "Spoof Admin",
                description: "Spoofs being an admin",
                run: function () {
...
alert('Remember, its just a spoof')
            ...
            {
                name: "Dark Mode",
                description: "Makes blooket dark mode",
                run: function () {
            ...
            },
            {
                ...
                run: function () {
                    if (console_msg("Op Fossils")) {
                        Object.values(document.querySelector('#app > div > div'))[1].children[0]._owner.stateNode.setState({ "fossilMult":1000000})
                        }
                }
            },
            {
        ...
        doom: [
            {
                name: "Megalodon op card",
                description: "Makes your current card op",
                run: function () {
                    window.setInterval(function(){
                        Object.values(document.querySelector('#app > div > div'))[1].children[0]._owner.stateNode.state.myCard = { blook: "Megalodon", charisma: 5, class: "🌊", strength: 1000, wisdom: 50}
                    })
                }
            },
            {
        ...
        fishing: [
            {
                name: "Always Frenzy",
                description: "Always frenzy",
                run: function () {
                    var { stateNode } = Object.values(document.querySelector('#app > div > div'))[1].children[0]._owner;
                    setInterval(() => {
                        stateNode.props.liveGameController.setVal({
                            path: `c/${stateNode.props.client.name}`,
                            val: {
        ...
        gold: [
            {
                name: "Op chests",
                description: "Makes your current chests op",
                run: function () {
                    setInterval(function(){var chests = Object.values(document.querySelector("#app > div > div"))[1].children[0]._owner.stateNode.state;
chests.choices = [{
"type": "multiply",
"val": 150,
"text": "Multiply Gold!",
...
"blook": "Rainbow Astronaut"
    ...
    for (let script of cheats.global) {
        ...
        cheat.style.backgroundColor = script.type == "toggle" ? script.enabled ? "#47A547" : "#A02626" : "hsl(0, 0%, 10%)";
        cheat.onclick = () => {
            try {
                script.run();
            } finally {
    ...
    const setCheats = (curPage) => {
        ...
        for (let cheat of cheats[curPage]) {
            ...
            button.style.backgroundColor = cheat.type == "toggle" ? cheat.enabled ? "#47A547" : "#A02626" : "hsl(0, 0%, 10%)";
            button.onclick = () => {
                try {
                    cheat.run();
                } finally {
    ...
    var offLocationChange = onLocationChange(pathname => {
        let curPage = getSite();
        curPageEl.innerText = getSite(true) ? `Gamemode: ${getSite(true)}` : 'Gamemode: No game detected';
        cheatDiv.innerHTML = "";
        setCheats(curPage);
    });
